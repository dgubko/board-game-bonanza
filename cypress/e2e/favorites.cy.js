...
describe("Top 100", () => {
  ...
  it("displays a title, header, and nav bar", () => {
    ...
    cy.get("#fav-button").should(
      "have.css",
      "background-color",
      "rgb(171, 176, 177)"
    );
    cy.get("#top100-button").should(
      "have.css",
      "background-color",
      "rgb(0, 221, 255)"
    );
  ...
  it("should display favorited games", () => {
    cy.get(".fav-game-card-wrapper")
      ...
      .within(() => {
        ...
        cy.get(".fav-game-card").within(() => {
          cy.get("#fav-image").should(
            "have.attr",
            "src",
            "https://s3-us-west-1.amazonaws.com/5cc.images/games/uploaded/1540147295104"
          );
          cy.get(".name-and-rating").within(() => {
  ...
  it("should display a message when no favorites exist", () => {
    ...
    cy.get(".heart-container").eq(0).click();
    cy.contains(
      "You currently have no favorite games. Go and favorite some cool games!"
    );
  });
});
